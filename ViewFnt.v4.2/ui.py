...
class FontRendererWidget(QWidget):
    def __init__(self, font):
        ...
        self.background_color = (50, 50, 50, 255)  # رنگ پیش‌فرض
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # اضافه کردن دکمه تغییر رنگ و اسلایدر زوم
        hbox = QHBoxLayout()
    ...
    def change_color(self):
        ...
        if color.isValid():
            # تبدیل QColor به tuple برای render_text
            self.background_color = (color.red(), color.green(), color.blue(), 255)
            self.on_text_changed(self.line_edit.text())

    def on_text_changed(self, text):
        img = self.font.render_text(text, background_color=self.background_color)
        if img is None:
            self.label.clear()
            return
